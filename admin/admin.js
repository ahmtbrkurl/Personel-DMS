...
async function post(data){
  ...
  if(!API_URL){
    throw new Error(
      "Google Apps Script API URL tanımlı değil."
    );
  }
...
function ensureFormBuilderDom(){
  ...
  if(top){
    ...
    if(!$("formGroup")){
      ...
      label.innerHTML=
        'Grup<select id="formGroup"></select>';
    ...
    if(!$("formName")){
      ...
      label.innerHTML=
        'Form Adı<input id="formName" value="Personel Başvuru Formu">';
    ...
    if(!$("formVersion")){
      ...
      label.innerHTML=
        'Versiyon<input id="formVersion" value="1.0">';
